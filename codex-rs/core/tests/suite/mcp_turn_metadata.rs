...
const REQUEST_USER_INPUT_METADATA_TEST_STACK_SIZE_BYTES: usize = 8 * 1024 * 1024;

fn run_metadata_request_user_input_test<F, Fut>(name: &'static str, test: F) -> Result<()>
where
    ...
    Fut: Future<Output = Result<()>> + 'static,
{
    let handle = std::thread::Builder::new()
        .name(name.to_string())
        .stack_size(REQUEST_USER_INPUT_METADATA_TEST_STACK_SIZE_BYTES)
        .spawn(move || -> Result<()> {
...
#[test]
fn mcp_tool_call_metadata_records_prior_request_user_input_tool() -> Result<()> {
    run_metadata_request_user_input_test(
        "mcp_tool_call_metadata_records_prior_request_user_input_tool",
        || async {
            skip_if_no_network!(Ok(()));
            ...
            let apps_tool_call =
                recorded_apps_tool_call_by_call_id(&server, calendar_call_id).await;

            assert_eq!(
                apps_tool_call.pointer(
                    "/params/_meta/x-codex-turn-metadata/user_input_requested_during_turn"
                ),
                Some(&json!(true))
            );
